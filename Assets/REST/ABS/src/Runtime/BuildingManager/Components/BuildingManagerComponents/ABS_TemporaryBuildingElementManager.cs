...
{
    public class ABS_TemporaryBuildingElementManager : ABS_BuildingManagerComponentBaseMonoBehaviour
    {
        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        //  properties
        //++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        {
            ABS_TemporaryBuildingElement tmpElment = m_Elements[p_X][p_Z];
            return tmpElment == null ? null : tmpElment.ValidationData;
        }
        ...
        public ABS_SimpleBuildingProcessErrorCode GetBlockErrorCode()
        {
            switch (m_FirstElement.BlockState)
            {
                case ABS_TemporaryBuildingElement.ABS_BlockState.BLOCKED_NOT_ENOUGH_MATERIAL:
                    return ABS_SimpleBuildingProcessErrorCode.RuleBreak_AboveMaximumElementCount;
                case ABS_TemporaryBuildingElement.ABS_BlockState.BLOCKED_PLAYER_COLLISION:
                    return ABS_SimpleBuildingProcessErrorCode.RuleBreak_PlayerCollision;
                case ABS_TemporaryBuildingElement.ABS_BlockState.BLOCKED_BUILDING_AREA:
                    return ABS_SimpleBuildingProcessErrorCode.RuleBreak_AreaRules;
                case ABS_TemporaryBuildingElement.ABS_BlockState.BLOCKED_BUILDING_LOGIC:
                default:
                    return ABS_SimpleBuildingProcessErrorCode.Successful;
            }
        }
        ...
        public ABS_TemporaryBuildingElementManager() : base()
        {
        ...
        public bool CreateElement(in int p_XByHitpoint, in int p_ZByHitpoint, in int p_CurrentXIndex, in int p_CurrentZIndex, in Vector3 p_DragGridSize)
        {
            ...
            {
                element.ValidationData = GetFirstElementResult();
            }
            else
            {
                ABS_PositionValidationData positionValidationResult = m_Manager.ValidatePosition(element.transform.position, element.transform.rotation);
                element.ValidationData = positionValidationResult;
            }
        ...
        private ABS_BuildingElement InstantiateImpl()   
        {
            ...
            else
            {
                ...
                if (element == null)
                {
                    REST_Logging.Warrning("ABS_TemporaryBuildingElementManager", "Get null element returned!");
        ...
        private ABS_PositionValidationData GetFirstElementResult()
        {
            ...
            {
                return ABS_PositionValidationData.PositionValidationDataFactory(m_ActiveBuildingElement);
            }
        }
...
#if UNITY_EDITOR
        public void DrawGizmos(ABS_ProjectSettings p_ProjectSettings)
        {
            if (p_ProjectSettings.DragBuilding_Index)
            {
                for (int x = 0; x < GetDimensionX(); ++x)
                {
                    for (int z = 0; z < GetDimensionZ(x); ++z)
                    {
                        ABS_TemporaryBuildingElement element = m_Elements[x][z];
                        if (element != null)
                        {
                            if (element.TargetBuildingElement.PositionSearchAlgorithm == ABS_PositionSearchAlgorithm.AdvancedGrid)
                            {
                                //TODO: fix the first element can be null in this case
                                ...
                                {
                                    REST_GizmosUtils.DrawText(element.transform.position,
                                        $"({x};{z}):{(element.ValidationData as ABS_PositionValidationData_AdvancedGrid).m_DragBuildingTemporaryStability}",
